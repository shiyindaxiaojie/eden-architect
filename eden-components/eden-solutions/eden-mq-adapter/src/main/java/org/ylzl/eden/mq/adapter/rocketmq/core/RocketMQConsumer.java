package org.ylzl.eden.mq.adapter.rocketmq.core;
...
import org.ylzl.eden.mq.adapter.rocketmq.autoconfigure.RocketMQMessageQueueAutoConfiguration;
...
public class RocketMQConsumer implements InitializingBean, DisposableBean {
	...
	private DefaultMQPushConsumer createConsumer(MessageQueueConsumer messageQueueConsumer) throws MQClientException {
		...
		if (StringUtils.isBlank(annotation.type()) &&
			!RocketMQMessageQueueAutoConfiguration.TYPE.equalsIgnoreCase(messageQueueProperties.getType())) {
			return null;
		}
