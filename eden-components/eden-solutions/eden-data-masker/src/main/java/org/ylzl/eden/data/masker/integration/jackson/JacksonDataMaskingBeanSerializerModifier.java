...
public class JacksonDataMaskingBeanSerializerModifier extends BeanSerializerModifier {
	...
	public List<BeanPropertyWriter> changeProperties(SerializationConfig config, BeanDescription beanDesc, List<BeanPropertyWriter> beanProperties) {
		...
		for (BeanPropertyWriter writer : beanProperties) {
			Masking masking = writer.getAnnotation(Masking.class);
			if (masking != null && writer.getType().isTypeOrSubTypeOf(String.class)) {
				JsonSerializer<Object> serializer = new JacksonDataMaskingJsonSerializer(masking, writer.getSerializer());
