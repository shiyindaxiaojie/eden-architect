 ...
 */
@Slf4j
public final class CustomLicenseManager extends LicenseManager {

	private static final int DEFAULT_BUFSIZE = 8 * 1024;

	private final TrueLicenseConfig trueLicenseConfig;

	public CustomLicenseManager(TrueLicenseConfig trueLicenseConfig) {
		...
		if (isPublicKey) {
			keyStoreParam =
				new CustomKeyStoreParam(
					CustomLicenseManager.class,
					trueLicenseConfig.getPublicKeysStorePath(),
					trueLicenseConfig.getPublicAlias(),
					trueLicenseConfig.getStorePass(),
					null);
		} else {
			keyStoreParam =
				new CustomKeyStoreParam(
					CustomLicenseManager.class,
					trueLicenseConfig.getPrivateKeysStorePath(),
					trueLicenseConfig.getPrivateAlias(),
					trueLicenseConfig.getStorePass(),
					trueLicenseConfig.getKeyPass());
		}
		Preferences preferences = Preferences.userNodeForPackage(CustomLicenseManager.class);
		CipherParam cipherParam = new DefaultCipherParam(trueLicenseConfig.getStorePass());
		LicenseParam licenseParam =
			new DefaultLicenseParam(
				trueLicenseConfig.getSubject(), preferences, keyStoreParam, cipherParam);
