...
package org.ylzl.eden.spring.integration.metrics.endpoint;
 ...
 */
@Slf4j
@WebEndpoint(id = AllMetricsEndpoint.ENDPOINT_ID)
public class AllMetricsEndpoint {

	public static final String ENDPOINT_ID = "allmetrics";

	private static final String MISSING_NAME_TAG_MESSAGE = "Missing name tag for metric {}";

	private final MeterRegistry meterRegistry;

	public AllMetricsEndpoint(MeterRegistry meterRegistry) {
					...
					operation -> {
						...
						Collection<Timer> httpTimersStream =
							this.meterRegistry
								...
								.timers();
						long count =
							httpTimersStream.stream().map(Timer::count).reduce((x, y) -> x + y).orElse(0L);
