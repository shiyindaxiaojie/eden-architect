...
Eden* Architect 是一组基于 Spring 体系的增强式框架，在官方各组件实现的基础上进行简化和扩展，消除样板代码和简化配置，开箱即用，降低初学者的使用门槛。

您可以通过 Eden* Architect 创建独立的 Java 应用程序，使用 `java -jar` 或更传统的 WAR 部署来启动它。

WARNING: Eden* Architect 使用的 Spring Boot 版本仅支持 JDK 1.8 以上，如果需要支持 JDK 1.7，请切换为 1.X.X 分支。
...
Eden* Architect 的源码支持在 JDK 1.8 以上的版本编译构建。
