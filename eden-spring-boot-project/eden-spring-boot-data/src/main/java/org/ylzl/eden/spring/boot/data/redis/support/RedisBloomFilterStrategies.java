...
enum RedisBloomFilterStrategies implements RedisBloomFilter.Strategy {
	...
	MURMUR128_MITZ_32() {
		...
		@Override
		public <T> boolean put(RedisTemplate<String, T> redisTemplate, String key, T value, int numHashFunctions,
			...
			for (int i = 1; i <= numHashFunctions; i++) {
				...
				if (!redisTemplate.opsForValue().setBit(key, combinedHash, true)) { // 通过合并 hash 值对 bit 的长度取模，所以数组的下标只能在 [0, bitSize) 区间
					return false;
				}
			}
			return true;
		...
		@Override
		public <T> boolean mightContain(RedisTemplate<String, T> redisTemplate, String key, T value, int numHashFunctions,
										Funnel<? super T> funnel) {
			long hash64 = Hashing.murmur3_128().hashObject(value, funnel).asLong();
			int hash1 = (int) hash64;
			int hash2 = (int) (hash64 >>> 32);
