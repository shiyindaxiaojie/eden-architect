...
public class RoutingDataSourceAutoConfiguration {
	...
	public static final String EXPS_ROUTING_DATASOURCE_ENABLED = "${" + DataConstants.PROP_PREFIX + ".routing-datasource.enabled:false}";
	...
	public static class InternalRoutingDataSourceAutoConfiguration implements ImportBeanDefinitionRegistrar, EnvironmentAware {
		...
		private static final String PROP_SPRING_DATA_PROPERTIES_DS_PREFIX = DataConstants.PROP_PREFIX + ".data-source";
		...
		private void setDefaultTargetDataSource(Environment env) {
			RelaxedPropertyResolver resolver = new RelaxedPropertyResolver(env, StringUtils.join(DataConstants.PROP_PREFIX, StringConstants.DOT));
			Map<String, Object> properties = new HashMap<>();
			properties.put(PROP_DATASOURCE_TYPE, resolver.getProperty(PROP_DATASOURCE_TYPE));
			properties.put(PROP_DATASOURCE_DRIVEE_CLASS_NAME, resolver.getProperty(PROP_DATASOURCE_DRIVEE_CLASS_NAME));
		...
		private void setTargetDataSources(Environment env) {
			RelaxedPropertyResolver resolver = new RelaxedPropertyResolver(env,
				StringUtils.join(PROP_SPRING_DATA_PROPERTIES_DS_PREFIX, StringConstants.DOT));
			...
			for (String key : keys) {
				...
				Map<String, Object> properties = resolver.getSubProperties(StringUtils.join(key, StringConstants.DOT));
				String enumName = ObjectUtils.toString(properties.get(PROP_DATASOURCE_ENUM),
					DataSourceEnum.SLAVE.name());
		...
		private DataSource buildDataSource(Map<String, Object> properties) {
			...
			return DataSourceBuilder.create()
				...
				.password(ObjectUtils.toString(properties.get(PROP_DATASOURCE_PASSWORD))).build();
		}
		...
		private void dataBinder(DataSource dataSource, Environment env) {
			...
			if (propertyValues == null) {
				Map<String, Object> subProperties = new RelaxedPropertyResolver(env, DataConstants.PROP_PREFIX).getSubProperties(StringConstants.DOT);
