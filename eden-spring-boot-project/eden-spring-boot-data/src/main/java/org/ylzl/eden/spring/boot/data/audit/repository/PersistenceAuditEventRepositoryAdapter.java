 ...
 * <ul>
 *     <li>{@code find(String principal, Date after, String type)} 变更为 {@code find(String principal, Instant after, String type)}</li>
 *     <li>{@code find(Date after)} 被移除</li>
 *     <li>{@code find(String principal, Date after)} 被移除</li>
 * </ul>
 *
 * @author gyl
 * @since 2.0.0
...
@Slf4j
@NoRepositoryBean
public abstract class PersistenceAuditEventRepositoryAdapter<T extends PersistentAuditEvent, ID extends Serializable> implements AuditEventRepository {

	private final PersistenceAuditEventRepository<PersistentAuditEvent, ID> persistenceAuditEventRepository;

	private final AuditEventConverter auditEventConverter;

	private int eventDataColumnMaxLength = 255;

	public PersistenceAuditEventRepositoryAdapter(PersistenceAuditEventRepository<PersistentAuditEvent, ID> persistenceAuditEventRepository,
												  AuditEventConverter auditEventConverter) {
		this.persistenceAuditEventRepository = persistenceAuditEventRepository;
		this.auditEventConverter = auditEventConverter;
	}

	public PersistenceAuditEventRepositoryAdapter(PersistenceAuditEventRepository persistenceAuditEventRepository,
												  AuditEventConverter auditEventConverter, int eventDataColumnMaxLength) {
		this.persistenceAuditEventRepository = persistenceAuditEventRepository;
		this.auditEventConverter = auditEventConverter;
