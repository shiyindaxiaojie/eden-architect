...
package org.ylzl.eden.spring.boot.security.jwt.token;
...
public class JwtTokenProvider {
    ...
    private static final String MSG_INVALID_SIGNATURE = "JWT signature are invalid, caught exception: {0}";
    ...
    private static final String MSG_UNSUPPORTED_TOKEN = "JWT token are unsupported, caught exception: {0}";

    private static final String MSG_HANDLE_EXCEPTION = "JWT token compact of handler are invalid, caught exception: {0}";
    ...
    @PostConstruct
    public void init() {
        this.secretKey = jwtProperties.getBase64Secret() != null ?
            jwtProperties.getBase64Secret() : Base64Utils.encodeToString(jwtProperties.getSecret().getBytes(StandardCharsets.UTF_8));
     ...
     */
    public Claims parse(String token) throws SecurityException, MalformedJwtException, ExpiredJwtException,
        ...
        try {
            ...
            throw new ExpiredJwtException(null, null, MessageFormat.format(MSG_EXPIRED_TOKEN, e.getMessage()), e);
        } catch (UnsupportedJwtException e) {
            throw new UnsupportedJwtException(MessageFormat.format(MSG_UNSUPPORTED_TOKEN, e.getMessage()), e);
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException(MessageFormat.format(MSG_HANDLE_EXCEPTION, e.getMessage()), e);
        }
    }
}
