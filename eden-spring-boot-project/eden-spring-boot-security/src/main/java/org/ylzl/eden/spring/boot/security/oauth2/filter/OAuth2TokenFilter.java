...
package org.ylzl.eden.spring.boot.security.oauth2.filter;
...
import org.ylzl.eden.spring.boot.framework.web.cookie.CookiesHttpServletRequestWrapper;
...
import org.ylzl.eden.spring.boot.security.oauth2.token.store.PersistentTokenCache;
...
public class OAuth2TokenFilter extends GenericFilterBean {
    ...
    private static final String MSG_UNAUTHORIZED_CLIENT_EXCEPTION = "Could not refresh OAuth2 token due to unauthorized";

    private static final String MSG_ACCESS_TOKEN_WITHOUT_REFRESH_TOKEN = "OAuth2 access token has expired, but there's no refresh token";
    ...
    public OAuth2TokenFilter(OAuth2CookieHelper oAuth2CookieHelper, TokenStore tokenStore, TokenGrantClient tokenGrantClient) {
    ...
    public HttpServletRequest refreshTokensIfExpiring(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) {
        ...
        if (mustRefreshToken(accessTokenCookie)) {
            ...
            if (refreshCookie != null) {
                try {
                    newHttpServletRequest = this.refreshToken(httpServletRequest, httpServletResponse, refreshCookie);
     ...
     * @return
     */
    private HttpServletRequest refreshToken(HttpServletRequest request, HttpServletResponse response, Cookie refreshCookie) {
