...
import org.ylzl.eden.spring.boot.framework.web.cookie.CookieCollection;
import org.ylzl.eden.spring.boot.framework.web.cookie.CookieHelper;
...
public class OAuth2CookieHelper {
    ...
    private static final List<String> COOKIE_NAMES = Arrays.asList(OAuth2Constants.ACCESS_TOKEN, OAuth2Constants.REFRESH_TOKEN, OAuth2Constants.SESSION_TOKEN);

    private static final long REFRESH_TOKEN_EXPIRATION_WINDOW_SECS = 3L;

    private static final String MSG_CREATE_COOKIE_IN_DOMAIN = "creating OAuth2 cookies for domain: {}";

    private static final String MSG_CREATE_ACCESS_TOKEN_COOKIE = "created OAuth2 access token cookie '{}'";

    private static final String MSG_CREATE_REFRESH_TOKEN_COOKIE = "created OAuth2 refresh token cookie '{}', age: {}";

    private static final String MSG_REMEMBER_REFRESH_TOKEN_COOKIE = "refresh OAuth2 token valid for another {} secs";

    private static final String MSG_SESSION_DURATION = "OAuth2 session duration {} secs, will timeout at {}";
     ...
     * @param oAuth2Cookies
     */
    public void createCookies(HttpServletRequest request, OAuth2AccessToken accessToken, boolean rememberMe, OAuth2Cookies oAuth2Cookies) {
        String domain = getCookieDomain(request);
        log.debug(MSG_CREATE_COOKIE_IN_DOMAIN, domain);

        Cookie accessTokenCookie = CookieHelper.create(OAuth2Constants.ACCESS_TOKEN, accessToken.getValue());
        ...
        log.debug(MSG_CREATE_REFRESH_TOKEN_COOKIE, refreshTokenCookie.getName(), refreshTokenCookie.getMaxAge());
