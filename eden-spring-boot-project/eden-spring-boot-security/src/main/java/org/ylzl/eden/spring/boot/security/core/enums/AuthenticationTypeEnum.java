...
package org.ylzl.eden.spring.boot.security.core.enums;
...
public enum AuthenticationTypeEnum {
    ...
    DIGEST_AUTH("Digest username=\"{0}\", realm=\"{1}\", nonce=\"{2}\", uri=\"/\", algorithm=\"{3}\", response=\"{4}\", opaque=\"{5}\""),
    HAWK_AUTHENTICATION("Hawk id=\"{0}\", ts=\"{1}\", nonce=\"{2}\", ext=\"{3}\", mac=\"{4}\""),
    AWS_SIGNATURE("AWS4-HMAC-SHA256 Credential={0}/{1}/{2}/{3}/aws4_request, SignedHeaders=content-type;host;x-amz-date;x-amz-security-token, " +
        "Signature={5}"),
    NTLM_AUTHENTICATION(""),
    OAUTH1("OAuth realm=\"{0}\",oauth_consumer_key=\"{1}\",oauth_token=\"{2}\",oauth_signature_method=\"{3}\",oauth_timestamp=\"{4}\"," +
