...
import lombok.experimental.UtilityClass;
import org.ylzl.eden.spring.boot.commons.env.os.OSArchEnum;
import org.ylzl.eden.spring.boot.commons.env.os.OSEnum;
...
public class OSUtils {
    ...
    private static final String[] LINUX_CPU_SHELL = {"/bin/bash", "-c", "dmidecode -t processor | grep 'ID' | awk -F ':' '{print $2}' | head -n 1"};

    private static final String[] LINUX_MAINBOARD_SHELL = {"/bin/bash", "-c", "dmidecode | grep 'Serial Number' | awk -F ':' '{print $2}' | head -n 1"};
    ...
    private static String getSerialNumber(String windowsMainboardCmd, String[] linuxMainboardShell) throws IOException {
        String serialNumber = null;
        Runtime runtime = Runtime.getRuntime();
        Process process = isWindows() ? runtime.exec(windowsMainboardCmd) : runtime.exec(linuxMainboardShell);
