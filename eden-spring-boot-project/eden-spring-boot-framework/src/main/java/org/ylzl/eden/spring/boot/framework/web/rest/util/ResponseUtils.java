...
public final class ResponseUtils {
	 ...
	 * @throws IOException
	 */
	public static long download(HttpServletRequest request, HttpServletResponse response, String srcPath) throws IOException {
		File srcFile = new File(srcPath);

		String fileName = srcFile.getName();
		response.setHeader(HttpHeaders.CONTENT_DISPOSITION, MessageFormat.format(CONTENT_DISPOSITION_ATTACH,
			URLEncoder.encode(fileName, FrameworkConstants.DEFAULT_ENCODING)));
		...
		if (StringUtils.isNotBlank(resolveRange(request))) {
			...
			response.setHeader(HttpHeaders.CONTENT_RANGE, StringUtils.join(ACCEPT_RANGES, StringConstants.SPACE,
				startByte, StringConstants.MINUS, endByte, StringConstants.SLASH, fileLength));
	...
	public static String resolveRange(HttpServletRequest request) {
		...
		if (range != null && range.contains(StringUtils.join(ACCEPT_RANGES, StringConstants.EQ))
	...
	public static long resolveStartBytesFromRange(HttpServletRequest request) throws NumberFormatException {
		String range = resolveRange(request);
		if (StringUtils.isBlank(range)) {
			return -1;
		}
		long startByte = 0;
		String[] ranges = range.substring(range.lastIndexOf(StringConstants.EQ) + 1).trim().split(StringConstants.MINUS);
		if (ranges.length == 1) {
	...
	public static long resolveEndBytesFromRange(HttpServletRequest request) throws NumberFormatException {
		String range = resolveRange(request);
		if (StringUtils.isBlank(range)) {
			return -1;
		}
