...
@ConditionalOnProperty(prefix = JavaMelodyConstants.PROP_PREFIX, name = "enabled", matchIfMissing = true)
...
public class JavaMelodyAutoConfiguration {
    ...
    public FilterRegistrationBean monitoringFilter(JavaMelodyProperties properties, ServletContext servletContext) {
        ...
        final FilterRegistration filterRegistration = servletContext.getFilterRegistration(BEAN_DEFAULT_NAME);
        if (filterRegistration != null) {
            registrationBean.setEnabled(false);
            for (final Entry<String, String> entry : registrationBean.getInitParameters()
                .entrySet()) {
    ...
    @ConditionalOnProperty(prefix = JavaMelodyConstants.PROP_PREFIX, name = "advisor-auto-proxy-creator-enabled", matchIfMissing = false)
    ...
    public SpringDataSourceBeanPostProcessor monitoringDataSourceBeanPostProcessor(
        @Value("${" + JavaMelodyConstants.PROP_PREFIX + ".excluded-datasources:}") String excludedDatasources) {
    ...
    @Bean
    @ConditionalOnProperty(prefix = JavaMelodyConstants.PROP_PREFIX, name = "spring-monitoring-enabled", matchIfMissing = true)
    ...
    @Bean
    @ConditionalOnProperty(prefix = JavaMelodyConstants.PROP_PREFIX, name = "scheduled-monitoring-enabled", matchIfMissing = true)
    @ConditionalOnMissingBean(DefaultAdvisorAutoProxyCreator.class)
    ...
    public MonitoringSpringAdvisor monitoringFeignClientAdvisor() throws ClassNotFoundException {
        final Class<? extends Annotation> feignClientClass = (Class<? extends Annotation>) Class
            .forName("org.springframework.cloud.netflix.feign.FeignClient");
