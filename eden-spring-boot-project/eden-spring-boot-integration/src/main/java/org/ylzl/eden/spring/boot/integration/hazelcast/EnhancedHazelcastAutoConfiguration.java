...
public class EnhancedHazelcastAutoConfiguration implements DisposableBean {
	...
	public static final String EXPS_HAZELCAST_ENABLED = "${" + FrameworkConstants.PROP_PREFIX + ".hazelcast.enabled:true}";

    private static final String DEFAULT_MAP_CONFIGS_KEY = "default";

    private static final String DEFAULT_DOMAIN_PATTERN = "domain.*";

    private static final String MSG_INJECT_HAZELCAST = "Inject Hazelcast";

    private static final String MSG_INJECT_HAZELCAST_MGR = "Inject Hazelcast CacheManager";
    ...
    public EnhancedHazelcastAutoConfiguration(EnhancedHazelcastProperties enhancedHazelcastProperties) {
    ...
    public HazelcastInstance hazelcastInstance(InfoContributorProvider infoContributorProvider) {
        log.debug(MSG_INJECT_HAZELCAST);
        HazelcastInstance hazelCastInstance = Hazelcast.getHazelcastInstanceByName(applicationName);
        if (hazelCastInstance != null) {
    ...
    public CacheManager hazelcastCacheManager(HazelcastInstance hazelcastInstance) {
        log.debug(MSG_INJECT_HAZELCAST_MGR);
	...
	protected Config initializeDefaultConfig(InfoContributorProvider infoContributorProvider) {
        ...
        config.getMapConfigs().put(infoContributorProvider.resolvePackage(DEFAULT_DOMAIN_PATTERN), this.initializeDomainMapConfig());
	...
	protected ManagementCenterConfig initializeDefaultManagementCenterConfig() {
        ...
        managementCenterConfig.setEnabled(enhancedHazelcastProperties.getManagementCenter().isEnabled());
        managementCenterConfig.setUrl(enhancedHazelcastProperties.getManagementCenter().getUrl());
        managementCenterConfig.setUpdateInterval(enhancedHazelcastProperties.getManagementCenter().getUpdateInterval());
