...
import org.ylzl.eden.spring.boot.commons.env.OSUtils;
...
public final class EnhancedLicenseManager extends LicenseManager {
    ...
    public void setLicenseParam(boolean isPublicKey) {
        ...
        if (isPublicKey) {
            keyStoreParam = new EnhancedKeyStoreParam(EnhancedLicenseManager.class, trueLicenseProperties.getPublicKeysStorePath(),
                trueLicenseProperties.getPublicAlias(), trueLicenseProperties.getStorePass(), null);
        } else {
            keyStoreParam = new EnhancedKeyStoreParam(EnhancedLicenseManager.class, trueLicenseProperties.getPrivateKeysStorePath(),
                trueLicenseProperties.getPrivateAlias(), trueLicenseProperties.getStorePass(), trueLicenseProperties.getKeyPass());
        }
        ...
        LicenseParam licenseParam = new DefaultLicenseParam(trueLicenseProperties.getSubject(), preferences, keyStoreParam, cipherParam);
    ...
    private void validateLicenseContentExtra(LicenseContentExtra extra) throws LicenseContentException {
    ...
    private Object load(String encoded) {
        ...
        try {
            inputStream = new BufferedInputStream(new ByteArrayInputStream(encoded.getBytes(CharsetConstants.UTF_8)));
